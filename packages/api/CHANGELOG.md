...
- 9f23b7e5: Feat: Added a V2 endpoint for team management.
...
- ebfc2e80: Extend observability instrumentation to the remaining API surfaces using the
...
- bbc29859: Improve API observability instrumentation. Add a centralized tracing + metrics
...
- d11991b0: fix: enforce password complexity on team invite acceptance
...
- ba626ef9: Add `backgroundChart` support to number tiles in the external dashboards API (`/api/v2/dashboards`). Builder number tiles can now carry an optional background trend sparkline (`type` line or area, with an optional palette-token `color`) over the v2 REST API, matching the dashboard editor. Raw SQL number tiles do not support it.
